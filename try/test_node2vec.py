...
def generate_silver_labels_FAST(
    ...
    model,
    class_hierarchy,
    output_path_train="Silver/silver_node2vec.json",
    output_path_test="Silver/silver_node2vec.json"
):
    ...
    base_category_embeddings = get_embeddings(
        ...
        model=model,
        batch_size=64,
        save_path="Embeddings/labels_base_node2vec.pt",
    ...
    # Add nodes
    G.add_nodes_from(range(531))

    # Add edges parent→child (or undirected)
    for cid, rel in hierarchy_int.items():
        for p in rel["parents"]:
            G.add_edge(p, int(cid))
        for c in rel["children"]:
            G.add_edge(int(cid), c)
    ...
    model_struct = node2vec.fit(window=10, min_count=1, batch_words=4)

    # Extract structural embeddings
    emb_struct = np.zeros((531, 128))
    for i in range(531):
        emb_struct[i] = model_struct.wv[str(i)]

    emb_struct = torch.tensor(emb_struct, dtype=torch.float32)
    ...
    text_emb = base_category_embeddings

    # normalize
    text_emb = F.normalize(text_emb, dim=1)
    emb_struct = F.normalize(emb_struct, dim=1)

    emb_struct = emb_struct.to(device)
    proj_node2vec = nn.Linear(128, 768, bias=False).to(device)
    emb_struct_proj = proj_node2vec(emb_struct)  # maintenant (531, 768)

    # concat ou addition
    text_emb = text_emb.to(device)  # Déplace text_emb sur GPU
    combined = text_emb + emb_struct_proj   # (531, 768)
    combined = F.normalize(combined, dim=1)

    hierarchical_embeddings = propagate_hierarchy_simple(
        label_embeddings=combined,
    ...
    torch.save(hierarchical_embeddings, "Embeddings/labels_hierarchical_node2vec.pt")

    review_embeddings = get_embeddings(
        all_texts,
        model=model,
        batch_size=64,
        save_path="Embeddings/X_train_test_node2vec_mpn.pt",
        force_recompute=True
    )
    ...
    json.dump(silver_train_nohier,
          open("Silver/silver_train_node2vec_nohier.json", "w", encoding="utf-8"),
          indent=2, ensure_ascii=False)

    json.dump(silver_test_nohier,
            open("Silver/silver_test_node2vec_nohier.json", "w", encoding="utf-8"),
            indent=2, ensure_ascii=False)
...
silver_train_safe, silver_test_safe, silver_train_safe_nohier, silver_test_safe_nohier = generate_silver_labels_FAST(
    ...
    model,
    class2hierarchy,
    output_path_train="Silver/silver_train_node2vec.json",
    output_path_test="Silver/silver_test_node2vec.json"
)
