...
all_texts = list(id2text_train.values()) + list(id2text_test.values())
all_texts_clean = [preprocess_text(t) for t in all_texts]

examples = [InputExample(texts=[t, t]) for t in all_texts_clean] 
...
def generate_silver_labels_FAST(
    ...
    model,
    class_hierarchy,
    output_path_train="Silver/silver_train_finetuned_mpn.json",
    output_path_test="Silver/silver_test_finetuned_mpn.json"
):
    ...
    base_category_embeddings = get_embeddings(
        ...
        model=model,
        batch_size=64,
        save_path="Embeddings/labels_base_finetuned_mpn.pt",
    ...
    torch.save(hierarchical_embeddings, "Embeddings/labels_finetuned_new_mpn.pt")

    review_embeddings = get_embeddings(
        all_texts,
        model=model,
        batch_size=64,
        save_path="Embeddings/X_train_test_finetuned_mpn.pt",
        force_recompute=True
    )
    ...
    json.dump(silver_train_nohier,
          open("Silver/silver_train_finetuned_mpn_nohier.json", "w", encoding="utf-8"),
          indent=2, ensure_ascii=False)

    json.dump(silver_test_nohier,
            open("Silver/silver_test_finetuned_mpn_nohier.json", "w", encoding="utf-8"),
            indent=2, ensure_ascii=False)
